...
def filter_authors_by_nationalities(nationality):
    ...
    authors = Author.objects.filter(nationality=nationality)
    for a in authors:
        result.append(a.biography) if a.biography else f'{a.first_name} {a.last_name}'

    return '\n'.join(result)


def filter_authors_by_birth_year(start_year, end_year):
    authors = Author.objects.filter(birth_date__year__range=(start_year, end_year))
