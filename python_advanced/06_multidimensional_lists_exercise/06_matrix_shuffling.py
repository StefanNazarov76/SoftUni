...
def swap_elements(command, indices):
    if len(indices) == 4 and check_coordinates(indices) and command == 'swap':
...
while True:
    command_type, *coordinates = [int(el) if el.isdigit() else el for el in input().split()]
